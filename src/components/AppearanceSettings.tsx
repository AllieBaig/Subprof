                 ...
                 {[
                   ...
                   <div key={item.id} className="flex items-center justify-between">
                     ...
                     <button 
                       ...
                       className={`w-8 h-5 rounded-full relative transition-colors ${settings.visibility.navigation[item.id as keyof typeof settings.visibility.navigation] !== false ? 'bg-apple-blue' : 'bg-system-tertiary-label/30'}`}
                     >
                       <motion.div className="absolute top-1 left-1 bg-white w-3 h-3 rounded-full shadow-sm" animate={{ x: settings.visibility.navigation[item.id as keyof typeof settings.visibility.navigation] !== false ? 12 : 0 }} />
                     </button>
                   </div>
                 ))}
                 <div className="flex items-center justify-between opacity-50">
                   <p className="text-xs font-semibold text-system-label">Settings (Required)</p>
                   <div className="w-8 h-5 rounded-full bg-apple-blue relative">
                     <div className="absolute top-1 left-[13px] bg-white w-3 h-3 rounded-full" />
                   </div>
                 </div>
               </div>
             </div>
